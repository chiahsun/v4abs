...
def get_acceptor_from_file(tcl_filename):
    with open(tcl_filename) as f:
        ...
        if len(content) > 5:
            ...
            exit(2)
    return question_acceptor_without_name(token_name, handle_name, ctor_name, rule_list)


class question_acceptor_without_name(acceptor.acceptor_base):
    ...
    def get_acceptor_name(self):
        return "question_acceptor_without_name"
    ...
    def string_of_accept_handle_with_pos(self, pos):
        return ("%s %s = %s(%s())\n") %(self.get_handle_type_name(), self.get_handle_name_with_pos(pos)
                                       , self.get_handle_type_name(), self.get_ctor_name())
