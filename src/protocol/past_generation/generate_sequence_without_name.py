...
def get_acceptor_from_file(tcl_filename):
    with open(tcl_filename) as f:
        ...
        try:
            ...
            print "Error file", tcl_filename
            exit(2)
        if len(content) > 5:
            print "Error: star should use only one line for its star sequence in", tcl_filename
            print content
            exit(2)
    return sequence_acceptor_without_name(token_name, handle_name, ctor_name, rule_list)


class sequence_acceptor_without_name(acceptor.acceptor_base):
    ...
    def get_acceptor_name(self):
        return "sequence_acceptor_without_name"
    ...
    def generate_parser_cpp(self):
        ...
        for acc in self.get_acc_list():
            ...
            content += acc.get_handle_name_with_pos(pos)
            pos = pos + 1
# No need to report left tokens for without name
#        content += (");\n"
#                    "    if (pos != getTokenChildSize()) {\n"
#                    "        std::cerr << \"Warning: Not all tokens consumed in %s for sequence rule\" << std::endl;\n"
#                    "        reportChildFrom(pos);\n"
#                    "    }\n"
#                    "    return true;\n") %(self.get_token_name())
        content += (");\n"
    ...
    def generate_ast_header(self):
        ...
        for acc in self.get_acc_list():
            if pos !=0:
                content += ', '
            content += ("%s %s") %(acc.get_handle_type_name(), acc.get_handle_name_with_pos(pos))
            pos = pos + 1
        content += ") {\n"
        pos = 0
        for acc in self.get_acc_list():
            content += ("            _%s = %s;\n") % (acc.get_handle_name_with_pos(pos), acc.get_handle_name_with_pos(pos))
            pos = pos + 1
        content += "        }\n"
        content += "    };\n"
